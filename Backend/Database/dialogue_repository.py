...
async def get_pending_requests_for_user(*, user_id: uuid.UUID, limit: int = 50) -> List[dict]:
    ...
    def _select():
        return (supabase
                .table(DIALOGUE_REQUESTS_TABLE)
                .select("*")
                .eq("recipient_user_id", user_id_str)  # Show requests TO the user, do not include requests FROM the user
...
# Fetch a specific dialogue request row (for linking messages to requests)
